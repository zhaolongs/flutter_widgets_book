...
class ImageUtils{
  ...
  void saveImageByUrl(String picNetUrl,{String picName,String endFormat="png",bool isReplace=true,bool isEncode=true}) async{
    ...
    ///参数一 图片的网络路径
    Response response = await _dio.get(picNetUrl,options: Options(responseType: ResponseType.bytes));
    ///读取图片信息
  ...
  Future<Uint8List> getImageUint8ListByPath(String picName,{String endFormat="png",bool isEncode=true}) async{
    ///先获取文件
    var file =  await getImageFileByPath(picName,endFormat: endFormat,isEncode: isEncode);
    if(file==null){
      return null;
    }else{
      bool exist =  await file.exists();
  ...
  ///[endFormat]文件（图片）对应的格式
  /// return File
  Future<File> getImageFileByPath(String picName,{String endFormat="png",bool isEncode=true}) async{
    //安全校验
    if(picName==null||picName.trim().length==0){
      return null;
    }
